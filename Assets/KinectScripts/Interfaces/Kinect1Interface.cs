...
public class Kinect1Interface : DepthSensorInterface 
{
	...
	private static extern int LoadSpeechGrammarNative([MarshalAs(UnmanagedType.LPWStr)]string sFileName, short iNewLangCode);
...
//							if(leftHandEvent != InteractionHandEventType.None)
//							{
//								Debug.Log(string.Format("{0} L - {1} {2}", skeletonId, leftHandState, leftHandEvent));
//							}
							
							uint rightHandState = GetRightHandState(intIndex);
							InteractionHandEventType rightHandEvent = GetRightHandEvent(intIndex);
							
//							if(rightHandEvent != InteractionHandEventType.None)
//							{
//								Debug.Log(string.Format("{0} R - {1} {2}", skeletonId, rightHandState, rightHandEvent));
//							}
...
//			}
	...
	{
		return LoadSpeechGrammarNative(sFileName, iLangCode);
	}
